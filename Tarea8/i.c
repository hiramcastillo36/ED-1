 ...
 * Descripción: Dada una secuencia A de N enteros y una secuencia B de M enteros, escribe un programa que
 *              determine qué elementos de B aparecen en A.
 */
...
void solve(int size_a, int size_b, int array_a[], int array_b[]) {
    quicksort(0, size_a-1, array_a);
    for(int i=0; i<size_b; i++) {
        bool answer = binary_search(size_a-1, array_b[i], array_a);
        array_b[i] =  answer;
    }
}

bool binary_search(int size, int searching, int array[]) {
